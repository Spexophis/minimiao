...
class ThorCMOS:
    ...
    def prepare_data_acquisition(self, n, fd, fn):
        """
        Configure the camera for a saving DPC acquisition. Reconstructed DPC
        images are buffered in stacks of n and written to fd as fn_dpc_*.tiff,
        along with the raw frames they came from as fn_raw_*.tiff.
        """
        self.set_roi()
        self.set_exposure_time()
        self.data = run_threads.DPCCameraDataList(max_length=n, save_to_disk=True,
                                                  save_dir=fd, file_prefix=fn)
        self.acq_thread = run_threads.CameraAcquisitionThread(self)
