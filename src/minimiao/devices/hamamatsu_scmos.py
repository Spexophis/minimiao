...
class HamamatsuCamera:
    ...
    def _initialize_api(self):
        ...
        if re:
            ...
            self.logg.info("Found {} DCAM".format(n))
            dcm = Dcam(0)
            output = '#{}: '.format(0)
            model = dcm.dev_getstring(DCAM_IDSTR.MODEL)
            if model is False:
                output = output + 'No DCAM_IDSTR.MODEL'
            else:
                output = output + 'MODEL={}'.format(model)
            camera_id = dcm.dev_getstring(DCAM_IDSTR.CAMERAID)
            if camera_id is False:
                self.logg.error(output + ', No DCAM_IDSTR.CAMERAID')
            else:
                self.logg.info(output + ', CAMERAID={}'.format(camera_id))
            re = dcm.dev_open()
            if re is not False:
                properties = {}
                id_prop = dcm.prop_getnextid(0)
                while id_prop is not False:
                    prop_name = dcm.prop_getname(id_prop)
                    if prop_name is not False:
                        properties[prop_name] = id_prop
                    id_prop = dcm.prop_getnextid(id_prop)
                return dcm, properties
            else:
                self.logg.error('Error: Failed to open the camera with error {}'.format(dcm.lasterr()))
                return None, None
        else:
            self.logg.error('Error: Failed to initialize dcamapi with error {}'.format(Dcamapi.lasterr()))
            return None, None
    ...
    def close(self):
        ...
        if re is not False:
            self.logg.info("DCAM device closed")
        else:
            self.logg.error("Failed to close DCAM device")
        re = Dcamapi.uninit()
        if re is not False:
            self.logg.info("DCAM API closed")
        else:
