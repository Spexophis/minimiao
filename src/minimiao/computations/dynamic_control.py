...
============================================================
Optimized version of DynamicControl for Shack-Hartmann WFS + deformable mirror.

Optimizations and fixes relative to the original code:
-------------------------------------------------------

1. DISCRETE vs CONTINUOUS Riccati equation
   ORIGINAL: Used solve_continuous_are() but the Kalman filter is discrete-time.
   FIX:      Use solve_discrete_are() — the system runs at a fixed frame rate,
             so state transitions are inherently discrete.

2. NUMERICALLY STABLE Kalman update
   ORIGINAL: P_post = (I - K·C) · P_pre  — this "simple" form accumulates
             roundoff and can lose positive-definiteness over time.
   FIX:      Use the Joseph stabilized form:
             P_post = (I - K·C) · P_pre · (I - K·C)^T + K · R · K^T
             This guarantees P stays symmetric positive-definite.

3. AVOID explicit matrix inverse
   ORIGINAL: K = P·C^T · inv(S)  — inv() is O(n³) and numerically fragile.
   FIX:      K = P·C^T · solve(S, I) using np.linalg.solve(), which is
             both faster and more stable (uses LU decomposition internally).

4. STEADY-STATE Kalman gain option
   ORIGINAL: Recomputes Kalman gain every frame — wasteful for a time-invariant
             system running at high frame rate.
   FIX:      Pre-compute the steady-state gain by iterating the DARE to
             convergence once at startup. Then each frame is just:
             x = A·x + K_ss·(z - C·A·x)  — a single matrix-vector multiply.
             This is critical for real-time AO (>100 Hz loop rates).

5. PROPER system matrix initialization for AO
   ORIGINAL: Random A, B, C, D — meaningless for a real system.
   FIX:      Provide an from_interaction_matrix() constructor that builds
             (A, B, C, D) from the measured DM influence functions:
             - B = interaction matrix (slopes per actuator volt)
             - C = identity or modal projection
             - A = temporal dynamics model (default: identity = static turbulence)
             - D = zero (no direct feedthrough in AO)

6. ADAPTIVE covariance with proper innovation-based estimation
   ORIGINAL: R = sample_cov(residuals), Q = alpha·I + (1-alpha)·Q  — the Q
             update is a blind heuristic with no physical basis.
   FIX:      Use the Mehra (1970) innovation-based method:
             R_new = C_yy - C · P_pre · C^T   (from innovation covariance)
             Q_new = K · C_yy · K^T            (from state prediction error)
             where C_yy = (1/N) Σ y·y^T  is the sample innovation covariance.
             This is a principled, well-known method from control theory.

7. SUBSPACE identification cleanup
   ORIGINAL: Hankel matrix construction had potential dimension mismatches
             and no validation of the identified model's stability.
   FIX:      Cleaner Hankel construction, explicit dimension checks, and
             optional spectral radius check on the identified A matrix.

8. SEPARATED gain tuning
   ORIGINAL: Single Q_lqr, R_lqr with no guidance on how to set them.
   FIX:      Expose a set_lqr_weights() method with physical interpretation:
             - Large Q_lqr penalizes wavefront error (aggressive correction)
             - Large R_lqr penalizes actuator stroke (conservative, avoids saturation)
             The ratio Q/R controls the aggressiveness of the controller.

9. LOGGING and diagnostics
   ORIGINAL: Stored residuals as a flat list with no structured diagnostics.
   FIX:      Track innovation statistics, actuator RMS, and loop convergence
             metrics in a structured diagnostics dict.

10. LEAKY INTEGRATOR fallback
    NEW:     Added a simple leaky integrator for comparison / fallback when
             the full LQG is not needed or the system model is uncertain.

Usage for AO wavefront correction:
    # Build from your measured interaction matrix
    ctrl = LQGController.from_interaction_matrix(
        interaction_matrix=IM,   # (n_slopes, n_actuators)
        n_modes=97,              # number of controlled modes
    )

    # In your control loop:
    for frame in camera_stream:
        slopes = measure_slopes(frame)
        actuator_cmd = ctrl.compute_control(slopes)
        send_to_dm(actuator_cmd)
"""

import numpy as np
from scipy.linalg import solve_discrete_are, svd, cho_factor, cho_solve
